...
volatile uint8_t  ctsu_data_index;

volatile uint8_t ts06_on_cnt;			// TS06のON回数 10msec毎にカウント　OFFになると0セット
volatile uint8_t ts07_on_cnt;			// TS07   :
...
void INT_ctsufn(void)
{
	...
	touch_key_proc();		// タッチキーを押している時に、ステッピングモータを動かす処理	
	
	touch_led_on_off();             //　キーのLEDのON/OFF 
...
//
// タッチキーを押している時に、ステッピングモータを動かす処理  (SW4優先)
// 
// ctsu_sc[0] : TS06:SW4 時計回り
// ctsu_sc[1] : TS08:SW3 反時計回り
//
void touch_key_proc(void)
{
	 if ( ctsu_sc[0] > TOUCH_ON_VALUE ) { // SW4 ON判定値より大きい場合、
	 ...
	 if ( ctsu_sc[1] > TOUCH_ON_VALUE ) { // SW3 ON判定値より大きい場合、
...
void touch_led_on_off(void)
{
	 ...
	 if ( ctsu_sc[2] > TOUCH_ON_VALUE ) { // SW2 ON判定値より大きい場合、
	 	P2_bit.no0 = 1;		      // P20 = 1 を出力
	 }
	 else{
		P2_bit.no0 = 0;		      // P20 = 0 を出力
	 }
	 
	  if ( ctsu_sc[3] > TOUCH_ON_VALUE ) { // SW1 ON判定値より大きい場合、
	 	P4_bit.no3 = 1;		      // P43 = 1 を出力
	 }
	 else{
