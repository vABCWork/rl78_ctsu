...
void INT_uart1_send(void)
{
	...
	P1_bit.no5 = 1;		// P15 = 1 , TXD1 LED 点灯	
	
	if ( send_pt == send_num ) { // 最終データの送信完了
           if ( sd_data[0] == 0xc0 ) {   // ctsu読み出しコマンドのレスポンスの場合
	   	STMK1 = 1;		// UART1 送信の転送完了、バッファ空き割り込みの禁止 (割り込みマスクセット)
		
		P1_bit.no5 = 0;		// P15 = 0 , TXD1 LED 消灯	
	   }
	}
}
...
void INT_uart1_rcv(void)
{
	...
	P1_bit.no4 = 1;		// P14 = 1 , RXD1 LED 点灯	
	...
	if ( rcv_data[0] == 0x40 ) {	// ctsu 読み出しコマンド	
 		if ( rcv_cnt == 8 ) {   //  合計 8 byte 受信で、受信完了
		  ...
		  P1_bit.no4 = 0;	// P14 = 0 , RXD1 LED 消灯	
 		}
	}
}
...
void comm_cmd(void)
{
      ...
      rcv_crc_code = buf_crc_cal( &rcv_data[0], 8);  // 全受信データ(8byte)のCRC計算
...
//	SREPR01 = 0;		// RXD1受信エラー割り込みレベル = 2 (レベル0～3 :0が最高)
//	SREPR11 = 1;
//	SREMK1 = 0;		// RXD1受信エラー割り込み許可 (割り込みマスククリア)
